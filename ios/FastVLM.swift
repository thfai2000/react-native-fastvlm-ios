...
private enum Language {
    ...
    fileprivate class Qwen2Model: Module {
        ...
        public func callAsFunction(
            ...
            let attMask: MLXArray? = createAttentionMask(h: h, cache: cache) // This should call the MLXArray? version
            
            for (i, layer) in layers.enumerated() {

                h = layer(h, mask: attMask, cache: cache?[i])
            }
...
public class FastVLMProcessor: UserInputProcessor {
    ...
    public func prepare(prompt: UserInput.Prompt, imageTHW: THW?) -> String {
        // Normalize the prompt into an array of message dictionaries.
        // The project's `Message` type is `[String: Any]`, so be flexible.
        typealias Message = [String: Any]
        var messages: [Message]
        switch prompt {
        case .text(let text):
            messages = [["role": "user", "content": text]]
        case .messages(let msgArray):
            // msgArray likely is [[String: Any]] (Message)
            messages = msgArray
        default:
            if let mirror = Mirror(reflecting: prompt).descendant("messages") as? [Message] {
                messages = mirror
            } else {
                messages = []
            }
        ...
        let firstRole = (messages.first?[("role")] as? String) ?? "user"
        if firstRole != "system" {
